class Liveness {
  constructor(videoWrapper, config) {
    ...
    if (config.luminanceControl)
      this.luminanceControl = config.luminanceControl;
    ...
    this.livenessConfirmEndpoint =
      config.livenessConfirmEndpoint || "/liveness/v2";
    this.ellipseStrokeStyleDefault = config.ellipseStrokeStyle || "#D02780";
    this.activatedEllipseStrokeStyle =
      config.activatedEllipseStrokeStyle || "#46E3C3";
    this.boxMessageBackgroundColor =
      config.boxMessageBackgroundColor || "#D02780";
  ...
  checkBrightness() {
    this.brightness = Math.floor(
      this.brightnessSum /
        (this.canvasLuminance.width * this.canvasLuminance.height)
    );
  }
      ...
      navigator.mediaDevices.getUserMedia = function (constraints) {
        ...
        if (!getUserMedia) {
          return Promise.reject(
            new Error("getUserMedia não implementado nesse browser")
          );
        }
    ...
    navigator.mediaDevices.enumerateDevices().then((devices) => {
      ...
      if (this.isMobile()) constraints.video = { facingMode: "user" };
        ...
        const backCamera = devices.filter(
          (device) =>
            device.kind === "videoinput" && !device.label.includes("m-de:vice")
        )[0];
  ...
  loop() {
    if (this.video.style.width.includes("%")) {
      ...
      this.displaySize.height = this.video.style.height =
    ...
    const frameBox = {
      ...
      height: Math.floor(this.config.height * 0.922),
    };
    frameBox.left = Math.floor(this.canvas.width / 2 - frameBox.width / 2);
    frameBox.top = Math.floor(
      this.videoWrapper.clientHeight / 2 - frameBox.height / 2
    );
    ...
    eyesOutter.left = Math.floor(
      frameBox.left + frameBox.width / 1.95 - eyesOutter.width / 1.95
    );
    ...
    eyesInner.left = Math.floor(
      frameBox.left + frameBox.width / 1.96 - eyesInner.width / 1.96
    );
    ...
    this.timer = setInterval(async () => {
      ...
      if (resizedDetections && resizedDetections.expressions) {
        ...
        if (expression !== "neutral") {
          if (this.config.isDebug) {
            this.blockMask(
              `Mantenha expressão neutra >> ${expression}`,
              canvasPosition,
              frameBox.left,
              frameBox.top,
              frameBox.height,
      ...
      if (resizedDetections.detection) {
        ...
        if (rotated) {
          if (this.config.isDebug) {
            this.blockMask(
              "Centralize seu rosto >> rotacionado",
              canvasPosition,
              frameBox.left,
              frameBox.top,
              frameBox.height,
        ...
        leftEye.frameBox.isInside = this.isInside(leftEye.meanPosition, {
          top: frameBox.top,
          left: frameBox.left,
          width: frameBox.width,
          height: frameBox.height,
        });
        rightEye.frameBox.isInside = this.isInside(rightEye.meanPosition, {
          top: frameBox.top,
          left: frameBox.left,
          width: frameBox.width,
          height: frameBox.height,
        ...
        leftEye.outterBox.isInside = this.isInside(leftEye.meanPosition, {
          top: eyesOutter.top,
          left: eyesOutter.left,
          width: eyesOutter.width,
          height: eyesOutter.height,
        });
        rightEye.outterBox.isInside = this.isInside(rightEye.meanPosition, {
          top: eyesOutter.top,
          left: eyesOutter.left,
          width: eyesOutter.width,
          height: eyesOutter.height,
        ...
        leftEye.innerBox.isInside = this.isInside(leftEye.meanPosition, {
          top: eyesInner.top,
          left: eyesInner.left,
          width: eyesInner.width,
          height: eyesInner.height,
        });
        rightEye.innerBox.isInside = this.isInside(rightEye.meanPosition, {
          top: eyesInner.top,
          left: eyesInner.left,
          width: eyesInner.width,
          height: eyesInner.height,
  ...
  takePicture() {
    ...
    const pictureData = context.getImageData(
      0,
      0,
      this.canvasBackground.width,
      this.canvasBackground.height
  ...
  checkBackground() {
    ...
    context.drawImage(
      ...
      0,
      0,
      this.canvasLuminance.width,
      this.canvasLuminance.height
    );
    const pictureData = context.getImageData(
      0,
      0,
    ...
    this.isBackgroundOK =
      this.brightness >= this.brightnessControl &&
      this.luminance >= this.luminanceControl;
        ...
        <p class="liveness-progress-text">
          ${
            this.uploadInProgress === 100
              ...
              : `Fazendo upload da selfie...<br /> (${this.uploadInProgress?.toFixed(
                  0
                )}% enviados)`
          }
        </p>
